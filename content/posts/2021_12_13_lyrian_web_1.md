...
<span class="positive large">イイ感じの自動作詞アプリがなかったので、作りました。</span>

そう、なければ作ればいいのです。簡単な話ですね。
...
あとは歌詞の生成と手動による修正を繰り返していけば、最後に歌詞が完成します。やったね！

## 何で作ったのか？

「イイ感じの作詞アプリがない」というお話でしたが、制作に至った経緯について簡単にお話します。
...
<span class="negative">でも後半はダメだろ。</span>明らかに「お前作詞放棄しただろ」感がビンビンに伝わってきます。

そこで思ったのですね。

<div class="center">
  <span class="positive medium">こんなん使うくらいだったら自分でアプリ作るわ！</span>
</div>
...
が、結局それは<u>学習させたデータの範囲内の話</u>になるので、そこに自らが求める要素を含んでいなければ生成できない、あるいは思ったような歌詞が生成されないわけですね。

AIを絡めた作詞アプリは、<u>難しいことなしにイイ感じの歌詞をサクッと作る</u>という用途には合うのかもしれませんが、一方で<span class="negative">柔軟性に欠けるところがある</span>という印象を受けました。
...
このように、Lyrianに突っ込んだ文章によって生成される歌詞が一変するので、<span class="positive">歌詞全体の雰囲気をコントロールできる柔軟性</span>という点については、LyrianはAI的な作詞アプリよりも優れています。
...
Lyrianの歌詞生成はランダム要素が非常に強いです。前後の文脈は一切考えずに、繋がりやすそうな単語をランダムに組み合わせているだけなので、<span class="positive">かなりユニークなフレーズを生み出すことがあります。</span>
...
ざっくりとは、1音符あたりに当てはめる音の情報量を増やすことで、<span class="positive">日本語が持つ平坦なリズムを解消し、リズミカルな歌詞を生成する</span>という機能を搭載しています。

一見すごそうに見えますが、単に機械的な計算によって詰め込めるか否かを判断しているだけです。単純な方法で判断しているため、<span class="negative">たまに変な歌詞の割り当てが出てくることがあります。</span>
