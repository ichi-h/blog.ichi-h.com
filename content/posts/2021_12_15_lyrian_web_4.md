...
{{<
  github-card
  url="https://github.com/ichi-h/cutup_jp"
  title="ichi-h/cutup_jp"
  description="日本語の文章をカットアップするWebアプリケーション。"
>}}
...
数あるUIライブラリ／フレームワークの中からReactを選定した理由ですが、ここに関しては単純に<b>慣れ</b>です。  
...
まず、なぜ今になってCSS Modulesを選択したのか。  
また、現在のフロントエンド開発における主要なスタイリング手法であるCSS in JSを選択しなかったのか。  
...
{{<
  github-card
  url="https://github.com/ichi-h/FastNotel/"
  title="ichi-h/FastNote"
  description="マークダウン形式のメモ管理Webアプリケーション"
>}}
...
現在、Lyrian Webのテストカバレッジは非常に低く、ロジックが複雑なところや、テストが書きやすいポイントに絞ってチェックしています。  
この記事を書いている今も少しずつテストを増やしている状態です。  
また、Cypressなどを用いたE2Eテスト、Percyなどを用いたビジュアルリグレッションテストも導入予定です。
...
今回は、この手の開発ツールで恐らく最も有名であろうMock Service Workerを使用することにしました。
...
想像以上に長くなったので今回はここまで。  
次回はコンポーネントの設計や開発で苦労したことなどについてお話します。
